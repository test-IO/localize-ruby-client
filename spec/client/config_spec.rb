  ...
  describe "#validate!" do # rubocop:disable Metrics/BlockLength
    ...
    context "when required attributes are missing" do
      ...
      it "raises a NameError when app_id is missing" do
        config.private_key = "test_private_key"
        config.root_path_to_save = "/path/to/save"
        expect { config.validate! }.to raise_error(NameError, "Error: uninitialized constant APP_ID in .env")
      end

      it "raises a NameError when private_key is missing" do
        config.app_id = "test_app_id"
        config.root_path_to_save = "/path/to/save"
        expect { config.validate! }.to raise_error(NameError, "Error: uninitialized constant PRIVATE_KEY in .env")
      end

      it "raises a NameError when root_path_to_save is missing" do
        config.app_id = "test_app_id"
        config.private_key = "test_private_key"
        expect { config.validate! }.to raise_error(NameError, "Error: uninitialized constant ROOT_PATH_TO_SAVE in .env")
      end
    end
  end
end
